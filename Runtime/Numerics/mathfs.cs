...
namespace Freya {
	...
	public static partial class mathfs {
		...
		public static int pointSideOfPlane( this inth2 p, inth2 planePos, inth2 planeNormal ) => p.pointSideOfPlane( planePos, planeNormal );
		public static int pointSideOfPlane( this int2 p, int2 planePos, int2 planeNormal ) => math.sign( math.dot( p - planePos, planeNormal ) );

		public static inth divideBy2( this int p ) => new() { h = p };
		public static inth2 divideBy2( this int2 p ) => new(p.x.divideBy2(), p.y.divideBy2());


		public static int signedQuadrant( this int2 v ) => v.quadrant() switch { 1 => +1, 2 => -2, 3 => -1, _ => 00 };

		public static int magChebyshev( this int2 v ) => math.max( math.abs( v.x ), math.abs( v.y ) );
		public static float magChebyshev( this float2 v ) => math.max( math.abs( v.x ), math.abs( v.y ) );
