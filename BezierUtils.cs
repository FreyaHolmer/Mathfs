...
public static class BezierUtils {
	...
	public static (float a, float b, float c) GetCubicDerivativeFactors( float p0, float p1, float p2, float p3 ) =>
	(
		3 * ( -p0 + 3 * ( p1 - p2 ) + p3 ),
		6 * ( p0 - 2 * p1 + p2 ),
		3 * ( -p0 + p1 )
	...
	public static (float a, float b) GetCubicSecondDerivativeFactors( float p0, float p1, float p2, float p3 ) =>
	(
		6 * ( -p0 + 3 * ( p1 - p2 ) + p3 ),
		6 * ( p0 - 2 * p1 + p2 )
	);
