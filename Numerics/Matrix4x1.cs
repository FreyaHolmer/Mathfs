...
namespace Freya {
	...
	[Serializable] public struct Matrix4x1 {
		...
		public float this[ int column ] {
			get => column switch { 0 => m0, 1 => m1, 2 => m2, 3 => m3, _ => throw new IndexOutOfRangeException( $"Matrix column index has to be from 0 to 3, got: {column}" ) };
			set {
				switch( column ) {
					case 0:
						m0 = value;
					...
					case 3:
						m3 = value;
						break;
					default: throw new IndexOutOfRangeException( $"Matrix column index has to be from 0 to 3, got: {column}" );
				}
			}
		}
